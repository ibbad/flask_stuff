...
class APITestCase(unittest.TestCase):
    ...
    def test_token_auth(self):
        ...
        response = self.client.get(url_for('api.get_posts'),
                            headers=self.api.get_api_headers('bad-token', ''))
        self.assertTrue(response.status_code==401)

        # get a token
        response = self.client.get(url_for('api.get_token'),
                headers=self.api.get_api_headers('john@example.com', 'cat'))
        self.assertTrue(response.status_code==200)
        json_response = json.loads(response.data.decode('utf-8'))
    ...
    def test_posts(self):
        ...
        response = self.client.get(url,
                    header=self.get_api_headers('john@example.com', 'cat'))
        self.assertTrue(response.status_code == 200)
        json_response = json.loads(response.data.decode('utf-8'))
        ...
        self.assertTrue(json_response['body_html'] == 'update body')
    ...
    def test_users(self):
        ...
        u2 = User(email='susan@example.com', username='susan', password='dog',
                  confirmed=True, role=r)
